...
def api_federation_status():
    """Get federation status from results"""
    import json
    import os
    
    # Try to read from results file
    results_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'part2-federated-research', 'results')
    summary_file = os.path.join(results_dir, 'summary_stats.json')
    ...
    return jsonify({
        'status': 'no_results',
        'has_results': False,
        'message': 'Run experiments first'
    })


@app.route('/api/federation/scenarios')
def api_federation_scenarios():
    """Get list of available scenarios"""
    return jsonify({
        'scenarios': [
            {'id': 'iid', 'name': 'IID', 'description': 'Same attack distribution'},
            {'id': 'non_iid', 'name': 'Non-IID', 'description': 'Different attack patterns'},
            {'id': 'zero_day', 'name': 'Zero-Day', 'description': 'New attack in later rounds'}
        ]
    })


@app.route('/api/federation/start', methods=['POST'])
def api_federation_start():
    """Start a federation experiment"""
    import subprocess
    import threading
    from datetime import datetime
    
    data = request.json or {}
    scenario = data.get('scenario', 'iid')
    rounds = data.get('rounds', 5)
    
    def run_experiment():
        try:
            # Change to research directory and run
            research_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'part2-federated-research')
            subprocess.run([
                'python', '-m', 'experiments.run_day4'
            ], cwd=research_dir, capture_output=True)
        except Exception as e:
            logger.error(f"Federation experiment error: {e}")
    ...
    return jsonify({
        ...
        'scenario': scenario,
        'rounds': rounds,
        'message': 'Federation started in background'
    })


@app.route('/api/federation/results/<scenario>')
def api_federation_scenario_results(scenario):
    """Get results for a specific scenario"""
    import json
    import os
    
    results_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'part2-federated-research', 'results')
    csv_file = os.path.join(results_dir, f'scenario_{scenario}.csv')
    summary_file = os.path.join(results_dir, 'summary_stats.json')
            ...
            with open(csv_file, 'r') as f:
                ...
                for row in reader:
                    result['rounds'].append({
                        ...
                        'rules': int(row.get('rules', 0)),
                        'accuracy': float(row.get('accuracy', 0)),
                        'global_rules': int(row.get('global_rules', 0))
                    })
        except:
            pass
...
def api_federation_global_rules():
    """Get global rules from federation"""
    import json
    import os
    ...
    # Check rule files
    rules_dir = os.path.join(os.path.dirname(__file__), 'federated', 'rules')
    
    global_rules = []
    
    # Read from any global rules file if exists
    for client_file in ['rules_client_A.txt', 'rules_client_B.txt', 'rules_client_C.txt']:
        rule_file = os.path.join(rules_dir, client_file)
    ...
    return jsonify({
        'global_rules': global_rules[:20],  # Limit to 20
